...
use diagram_commands::{
    BringToFrontPayload, CellTarget, Command, Editor, SendToBackPayload, Transaction,
};
...
fn transaction_bring_to_front_routes_through_builder() {
    ...
    let tx = Transaction::new().bring_to_front(CellTarget::Vertex(v1));
    assert_eq!(
        tx.pending(),
        1,
    ...
    assert_eq!(editor.model().store.vertex(v1).unwrap().z_order, 6);
}
...
fn transaction_send_to_back_routes_through_builder() {
    ...
    let mut editor = Editor::from_model(model);
    let tx = Transaction::new().send_to_back(CellTarget::Vertex(v2));
    assert_eq!(
        tx.pending(),
        1,
    ...
    assert!(
        editor.model().store.vertex(v2).unwrap().z_order < 1,
        "v2 should be at bottom after SendToBack, got z={}",
        editor.model().store.vertex(v2).unwrap().z_order
    );
}
...
fn all_four_z_order_commands_change_model_correctly() {
    ...
    Transaction::new()
        .bring_to_front(CellTarget::Vertex(v1))
        .commit(&mut editor)
        .expect("commit");
    assert_eq!(editor.model().store.vertex(v1).unwrap().z_order, 8);

    // SendToBack: set target z_order to min(page) - 1.
    // After bring_to_front(v1): v1=8, v2=3, v3=5, v4=7. Min=3, so v4→2.
    Transaction::new()
        .send_to_back(CellTarget::Vertex(v4))
        .commit(&mut editor)
        .expect("commit");
    assert_eq!(editor.model().store.vertex(v4).unwrap().z_order, 2);

    // BringForward on v3 — moves one step toward top by swapping with
    // the next-higher vertex. After the previous ops:
    //   v1=8 (top), v2=3, v3=5, v4=0 (bottom)
    // The next vertex above v3 (z=5) is v1 (z=8). BringForward(v3)
    // should put v3 above the next vertex above it.
    ...
    let z1_before = editor.model().store.vertex(v1).unwrap().z_order;
    Transaction::new()
        .send_backward(CellTarget::Vertex(v1))
        .commit(&mut editor)
        .expect("commit");
    let z1_after = editor.model().store.vertex(v1).unwrap().z_order;
    assert!(
        z1_after < z1_before,
...
fn send_to_back_payload_carries_target_kind_edge() {
    let payload: SendToBackPayload =
        SendToBackPayload::new(CellTarget::Edge(EdgeId::default()));
